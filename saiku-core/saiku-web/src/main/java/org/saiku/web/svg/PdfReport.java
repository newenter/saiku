...
public class PdfReport {
    ...
    public byte[] pdf(CellDataSet c, String svg) {
        ...
        try {
            ...
            document.addHeader("Saiku Export - " + dateFormat.format(date) + " Page: ", "Saiku Export - " + dateFormat.format(date) + " Page: ");
            ...
            if (aSection.getData() != null) {
                ...
                for (int t = 0; t < aSection.getData().length; t++) {
                    for (int x = aSection.getData()[0].length
                        ...
                        int r = t % 2;
                        if (r != 0) {
                            cell.setBackgroundColor(color(BaseColor.BLACK, (float) 0.92));
                        }
